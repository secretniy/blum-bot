...
class GameService {
  ...
  async playGame(user, delay) {
    try {
      ...
      if (data) {
        user.log.log(
          `Start playing the game, finish it and receive your reward later: ${colors.blue(
            delay + "s"
          )}`
        );
        return data.gameId;
  ...
  async claimGame(user, gameId, eligibleDogs) {
    ...
    try {
      ...
      if (data) {
        user.log.log(
          `After playing the game, get the reward: ${colors.green(
            points + user.currency
          )}${eligibleDogs ? ` - ${dogs} 🦴` : ""}`
        );
        return true;
      } else {
        throw new Error(`Get rewards for failed games: ${data.message}`);
      }
    } catch (error) {
      user.log.logError(
        `Get rewards for failed games: ${error.response?.data?.message}`
      );
      return false;
    }
  }
  ...
  async handleGame(user, playPasses, timePlayGame) {
    ...
    if (isInTimeRange) {
      ...
      user.log.log(
        colors.yellow(
          `Once installed, the game cannot be played during this period, play next time: ${colors.blue(
