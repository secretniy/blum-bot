...
class TribeService {
    ...
    try {
      ...
      if (data) {
        ...
        const tribeSkip = user?.database?.tribeSkip || [
          "2bda082c-dba2-4bac-89b9-f1ee514ce170",
        ];
        return top100
  ...
  async joinTribe(
    user,
    tribeId = "2bda082c-dba2-4bac-89b9-f1ee514ce170",
    skipLog = false
  ) {
    ...
    try {
      ...
      if (!skipLog) {
        user.log.logError(
          `Joining the tribe failed: ${error.response?.data?.message}`
        );
      }
    }
  }
